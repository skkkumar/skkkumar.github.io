...
function CanvasControl(canvas, elements, callbackFunc) {
  ...
  document.addEventListener( "keydown", function(event) {
    switch(event.which) {
			...
			updateSourcePosition(that, true);
			that.draw();
			break;

			case 39: // right
			console.log("right press");
			updateSourcePosition(that, false);
			that.draw();
			break;
			...
			if(!that.directionMode){
				...
				stopMusic();
				speakText(getOClock(that.angle), playMusic);
...
function updateSourcePosition(object, clockwiseTurn){
	...
	if (angle <0){
		angle = 360 - increment;
	}else if(angle > 360 - increment)
		angle = 0;
	
	
	//compute position
	var x = 0.5 + 0.25 * Math.cos(angle * Math.PI / 180);
	var y = 0.5 - 0.25 * Math.sin(angle * Math.PI / 180);
	
	
	//update position of source
	object._elements[0].x = x;
	object._elements[0].y = y;
	object.angle = angle;
...
function getOClock(angle){
	var clock = 0;
	//converts angle into clock hands
	if(angle <= 60)
		clock = 3 - angle/30;
	else
		clock = 15 - angle/30;
...
CanvasControl.prototype.draw = function() {
  ...
  for (let i = 0; i < this._elements.length; i++) {
	...
	ctx.beginPath();
	ctx.arc(222, 222, 100, 0, 2 * Math.PI);
	ctx.stroke();
